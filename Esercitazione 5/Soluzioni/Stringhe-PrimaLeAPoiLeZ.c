 ...
 * caratteri della stringa cos� che tutte le 'a' (minuscole) compaiono all'inizio della
...
void primaLeAPoiLeZ(char *stringa) {
	...
	while(i<j) {
		...
		if(stringa[i]=='a')
			...
			/* il carattere con indice i � diverso da 'a' e quello con indice
			 * j � una 'a': scambiali */
				char temp = stringa[i];
				stringa[i] = stringa[j];
				stringa[j] = temp;
	 ...
	 * rappresenta ancora l'indice del primo carattere che non � una 'a'
	...
	while(i<j) {
		...
		if(stringa[j]=='z')
			...
			/* il carattere con indice i � una 'z' e quello con indice
			 * j � diverso da 'z': scambiali */
				char temp = stringa[i];
				stringa[i] = stringa[j];
				stringa[j] = temp;
			}
...
void testPrimaLeAPoiLeZ() {
	...
	/* stringa in cui l'unica 'a' fuori posto � in ultima posizione */
	char stringa4[] = "0000a";
	primaLeAPoiLeZ(stringa4);
	printf("Test primaLeAPoiLeZ(\"0000a\"): Atteso = \"a0000\", Calcolato = \"%s\" \n", stringa4);

	/* stringa in cui l'unica 'z' fuori posto � in prima posizione */
	char stringa5[] = "z0000";
